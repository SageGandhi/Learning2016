		...
		(
				...
				function(window, noGlobal) {
					...
					(function(window) {
						 ...
						 * 
						 * @returns {Function(string, Object)} Returns the Object data after storing it on itself with property name the (space-suffixed) string and
						...
						Expr = Sizzle.selectors = {
							...
							preFilter : {
								...
								"CHILD" : function(match) {
									/*
									 * matches from matchExpr["CHILD"] 1 type (only|nth|...) 2 what (child|of-type) 3 argument (even|odd|\d*|\d*n([+-]\d+)?|...) 4
									 * xn-component of xn+y argument ([+-]?\d*n|) 5 sign of xn-component 6 x of xn-component 7 sign of y-component 8 y of y-component
									 */
					 ...
					 * 
					 * memory: will keep track of previous values and will call any callback added after the list has been fired right away with the latest
						...
						for (prop in props) {
							...
							if (rfxtypes.exec(value)) {
								...
								if (value === (hidden ? "hide" : "show")) {
									...
									// If there is dataShow left over from a stopped hide or show and we are going to proceed with show, we should pretend to be
									// hidden
					...
					/*
					 * Prefilters 1) They are useful to introduce custom dataTypes (see ajax/jsonp.js for an example) 2) These are called: - BEFORE asking for a
					 * transport - AFTER param serialization (s.data is a string if s.processData is true) 3) key is the dataType 4) the catchall symbol "*" can be
					 * used 5) execution will start with transport dataType and THEN continue down to "*" if needed
					 */
					prefilters = {},

					/*
					 * Transports bindings 1) key is the dataType 2) the catchall symbol "*" can be used 3) selection will start with transport dataType and THEN go
					 * to "*" if needed
					 */
					transports = {},
					...
					/*
					 * Handles responses to an ajax request: - finds the right dataType (mediates between content-type and expected dataType) - returns the
					...
					jQuery
							.extend({
								...
								ajaxSettings : {
									...
									/*
									 * timeout: 0, data: null, dataType: null, username: null, password: null, cache: null, throws: false, traditional: false,
									 * headers: {},
									 */
																...
																return access(
																		...
																		function(
																			...
																			if (elem.nodeType === 9) {
																				...
																				// unfortunately, this causes bug #3838 in IE6/8 only, but there is currently no
																				// good, small way to fix it.
																				return Math
																						.max(
																								elem.body["scroll"
																										+ name],
